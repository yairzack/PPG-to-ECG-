...
from utils.findHR import findHR
from utils.find_closest_HR import find_closest_pretrained_model
...
for idx in subject_ids:
    ...
    try:
        ...
        # --- Choose pre-trained model based on subject HR ---
        hr = findHR(ecg, calcTime=20, fs=125)  # Assuming fs is the nominal sampling frequency of 125 Hz
        print(f"Subject {file_idx} initial HR: {hr:.2f} bpm")
        model_path, model_hr, difference = find_closest_pretrained_model("pretrain_models", hr)
        model = ECG2PPGps()  # Ensure this matches the architecture used during training
        ...
        torch.save(model.state_dict(), f"subject_models/subject_{file_idx}_from_pretrain_model.pth")
        print(f"💾 Model saved: subject_models/subject_{file_idx}_from_pretrain_model.pth")
        ...
        eval_df.to_csv(f"subject_models/subject_{file_idx}_from_pretrain_evaluation.csv", index=False)

        print(f"💾 Evaluation saved: subject_models/subject_{file_idx}_from_pretrain_evaluation.csv")
