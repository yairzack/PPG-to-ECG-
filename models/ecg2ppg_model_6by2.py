...
class UpSampleBlock(nn.Module):
    def __init__(self, in_channels, skip_channels, out_channels):
        ...
        self.convs = nn.Sequential(
            nn.Conv1d(in_channels + skip_channels, out_channels, kernel_size=15, padding='same'),
            nn.BatchNorm1d(out_channels),
            nn.LeakyReLU(),
            nn.Conv1d(out_channels, out_channels, kernel_size=15, padding='same'),
            nn.BatchNorm1d(out_channels),
...
class ECG2PPGps(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, base_channels=24):
        ...
        for i in range(depth):
            ...
            self.down_blocks_left.append(DownSampleBlock(in_ch, out_ch))
        ...
        for i in reversed(range(depth)):
            ...
            self.up_blocks_left.append(UpSampleBlock(in_ch, skip_ch, out_ch))
        ...
        for i in range(depth):
            out_ch = base_channels + 24 * (i + 1)
            self.down_blocks_right.append(DownSampleBlock(in_ch, out_ch))
        ...
        for i in reversed(range(depth)):
            ...
            self.up_blocks_right.append(UpSampleBlock(in_ch, skip_ch, out_ch))
